  ...
  return (
    ...
    >
      ...
      <div className="rounded-[2rem] border border-slate-800 bg-slate-900/55 p-6 shadow-xl shadow-black/10 md:p-8">
        <div className="mb-8 rounded-[1.5rem] border border-slate-800 bg-slate-950/60 p-6 md:p-7">
          ...
          {JOURNEY_STEPS.map((step, index) => {
            ...
            return (
              <motion.div
                ...
                className={`rounded-[1.5rem] border ${color.border} ${color.panel} p-6 md:p-7`}
              >
                <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-4">
                    <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl border border-white/8 bg-slate-950/70 text-sm font-semibold text-white">
                      {step.id}
                    </div>
                    <div className="rounded-full border border-white/8 bg-slate-950/55 p-2.5">
                      <Icon size={18} className={color.icon} />
                    </div>
                    <span className={`inline-flex rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] ${color.badge}`}>
                      {step.badge}
                    </span>
                  </div>
                </div>
                ...
                <p className="mb-5 text-[15px] leading-7 text-slate-300">{step.description}</p>
